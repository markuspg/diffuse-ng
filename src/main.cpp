 ...
 */
  ...
  for (int i = 0; i < argc; ++i) {
    tmp_args.emplace_back(Glib::locale_to_utf8(argv[i]));
  }
  const std::vector<Glib::ustring> args{std::move(tmp_args)};
  ...
  if ((2 == argc) && ("-h" == args[1]) || ("--help" == args[1])) {
    Df::printMessage(
        "Usage:\n"
        "    diffuse [ [OPTION...] [FILE...] ]...\n"
...
# This class holds the set of supported version control systems.  Each version
# control system should implement:
#
#   __init__():
...
class VCSs:
    ...
    def __init__(self):
        ...
        self.leaf_dir_repos = [ ('CVS', VCSs.Cvs), ('RCS', VCSs.Rcs) ]
        self.common_dir_repos = [ ('.git', VCSs.Git), ('.hg', VCSs.Hg), ('.svn', VCSs.Svn), ('.bzr', VCSs.Bzr), ('_darcs', VCSs.Darcs), ('_MTN', VCSs.Mtn) ]
    ...
    def findByFolder(self, path, prefs):
        ...
        try:
            for s in os.listdir(path):
                if s.endswith(',v') and os.path.isfile(os.path.join(path, s)):
                    return VCSs.Rcs(path)
        except OSError:
            # the user specified an invalid folder name
            pass
        # $GIT_DIR takes precedence over .git folders
        if 'GIT_DIR' in os.environ:
            try:
                ...
                pass
        if self.isSvkManaged(path):
            return VCSs.Svk(path)
        while True:
    ...
    if (!arg.empty() && ('-' == arg[0])) {
      if ((i + 1 < argc) && (("-c" == arg) || ("--commit" == arg))) {
        ...
        try {
          ...
          std::cerr << "Could not parse passed line number\n";
        } catch (const std::out_of_range &e) {
          std::cerr << "Passed line number is out of range\n";
        }
